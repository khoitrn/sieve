...
1. Owner decision needed: the diagram's maintenance-lane "Validate" step claims a tier check that `scripts/validate-skill.mjs` does not perform, and no SKILL.md frontmatter carries a tier field (tier lives only in sieve.index.json). Decide whether to fix the diagram (drop the tier claim) or add a real index cross-check to validate.
2. The diagram labels the contrib lane "Staging (contrib/)"; the real directory is `staging/`. The PNG itself was not edited as part of this audit (out of scope for a text-editing pass); align it if the diagram is kept as the canonical picture.
